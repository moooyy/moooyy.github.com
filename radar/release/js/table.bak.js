...
(function () {
    ...
    var fields = [
        {
            // title : '选择',
            title : function () {
                return '<th><input class="allCheck" type="checkbox" checked></th>';
            },
            width : 40,
            content : function (data, index) {
                return '<input index=' + index + ' class="subCheck" type="checkbox" checked>';
            }
        },
        {
            title : '序号',
    ...
    function drawTable (tableData) {
        ...
        $("#tablearea").show();
        $('#finalTable .subCheck').bind('click', checkHandler);
        $('#finalTable .allCheck').bind('click', allCheckHandler);
        checkHandler();
    ...
    function checkHandler () {
        ...
        timer = setTimeout(function () {
            var indexList = [];
            $('#finalTable .subCheck:checked').each(function () {
                indexList.push($(this).attr('index'));
            });
            var data = radar.dataSource.get(indexList);
            radar.drawRadar(data, indexList);
        }, 500);
        var allCheck = true, notAllCheck = false;
        $('#finalTable .subCheck').each(function () {
            allCheck = allCheck && $(this).prop('checked');
            notAllCheck = notAllCheck || $(this).prop('checked');
        });
        $('#finalTable .allCheck').prop('checked', allCheck);
        $('#finalTable .allCheck')[0].indeterminate = !allCheck && notAllCheck;
    }
